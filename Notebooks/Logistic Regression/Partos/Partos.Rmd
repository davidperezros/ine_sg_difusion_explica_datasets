...
#1 es con cesarea
datos1$MULTIPLI<-as.factor(datos1$MULTIPLI)
datos1$NORMA<-as.factor(datos1$NORMA)




lmod2 <- glm(formula = CESAREA ~ EDADM+SEMANAS+MULTIPLI+NORMA,family = binomial(link = logit),data=datos1)
summary(lmod2)

library(Epi)
#The ROC function
ROC(form=CESAREA ~ EDADM+SEMANAS+MULTIPLI+NORMA, data=datos1,plot="ROC",lwd=3,cex=1.5)
...
datos1 %>% ggplot(aes(x = NORMA)) +  
  geom_density(aes(group = CESAREA, 
                   colour = CESAREA, 
                   fill = CESAREA),
               alpha = 0.2)

datos1 %>% ggplot(aes(x = EDADM)) +  
  geom_density(aes(group = CESAREA, 
                   colour = CESAREA, 
                   fill = CESAREA),
               alpha = 0.2)
...
library(klaR)
partimat(CESAREA ~ SEMANAS+EDADM, data=datos1, method="qda")
