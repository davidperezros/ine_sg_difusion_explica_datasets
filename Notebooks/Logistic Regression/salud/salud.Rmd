...
```{r, eval=FALSE}
# confusion matrices
predicted2 <- predict(lmod2, data.frame(Edad = datos$Edad, Altura = datos$Altura, Peso=datos$Peso), type = "response")
library(caret)
confusionMatrix(data = as.factor(ifelse(predicted2 > 0.51, 1, 0)), reference = datos$Sexo)
```
