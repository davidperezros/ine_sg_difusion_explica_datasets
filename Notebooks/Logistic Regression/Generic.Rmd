...
En el siguiente dataset disponemos de datos 

## Descripción del trabajo a realizar

**(Esto irá en la web de explica)** Se pretende hacer una Regresión Logística que clasifique .

  - Hacer un análisis exploratorio.
  - Plantear las hipótesis de una regresión logística (incluyendo todas variables).
  - Analizar el modelo planteado y su ajuste de bondad.
  - Comprobar hipótesis de regresión logística.
  - Hacer una conclusión.

## Técncnica Estadística

El objetivo es realizar una **Regresión Logística** para clasificar la
variable **indice_total** en función de las demás variables (**dim1,...,dim8**). Para ver su ajuste miraremos los gráficos pertinentes y las
...
La regresión logística es útil cuando se trabaja con una **variable categórica o binaria**, es decir, cuando la respuesta es sí/no, verdadero/falso o pertenece a una de dos categorías. En particular esta toma la forma: 

$$\eta_i=\beta_0+\beta_1 x_{i 1}+\beta_2 x_{i 2}+\ldots \beta_k x_{i k}$$ con $$\eta=\log \left(\frac{p}{1-p}\right) \Rightarrow p=\frac{e^\eta}{1+e^\eta}$$ siendo $p=P(Y=1)$.

Notar que curva logística es casi lineal en su rango medio (ver siguiente gráfico). Esto significa que al modelar respuestas con una probabilidad moderada, la regresión logística y la lineal no se comportarán de manera muy diferente. La curva se acerca a uno en su extremo superior y a cero en su extremo inferior, pero nunca alcanza estos límites. Esto implica que la regresión logística nunca predecirá que algo es inevitable o imposible. Mapea "p", que está limitado entre 0 y 1, a la línea de números reales. La proporción $p/(1 − p)$ en la transformación se llama **odds**.
...
set.seed(123)
x <- rnorm(100)
y <- rbinom(100, 1, plogis(0.01 + 10 * x))  # Simulating binary outcome variable
...
predictions <- predict(model, newdata = data.frame(x = x_seq), type = "response")

# Plotting the logistic regression curve
plot(x, y, col = "blue", main = "Logistic Regression Curve")
...
Para que una regresión logística proporcione un buen ajsute a los datos debe cumplir una serie de requisitos que por tanto deben ser verificados al llevar a cabo el estudio. 

Las hipótesis que se deben cumplir son:

  - **Respuesta binaria**: La variable respuesta debe tomar sólo 2 valores y los predictores deben ser variables ordinarias.    
  
- **Multicolinealidad**: Se debe verificar que no hay multicolinealidad entre las variables predictoras.


- **Forma funcional**: La regresión logística asume la **linealidad** de las variables independientes y el $log odds$.
